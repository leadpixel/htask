    ...
    match command {
        ...
        Commands::List {
            ...
            if cli.json {
                ...
                render_tasks(&filtered, show_uuid);
            }
        }
        Commands::Summary => {
            let tasks = store.list_tasks();
            ...
            if cli.json {
                ...
                if in_progress.is_empty() {
                    ...
                    for (i, t) in in_progress.iter().enumerate() {
                        print_task(i + 1, t, false);
                    }
                }
                ...
                for (i, t) in top_pending.iter().enumerate() {
                    print_task(i + 1 + in_progress.len(), t, false);
                }
            }
        }
    }
...
fn divider(label: &str) -> String {
    format!("{} {}", label.bold(), "─".repeat(60 - label.len()).dimmed())
}

fn render_tasks(tasks: &[htask::domain::Task], show_uuid: bool) {
    ...
    for (i, task) in tasks.iter().enumerate() {
        if Some(task.status) != current_status {
            ...
            println!(
                "\n{}",
                divider(&format!("{:?}", task.status).to_uppercase())
            );
        }
        print_task(i + 1, task, show_uuid);
    }
}

fn print_task(idx: usize, task: &htask::domain::Task, _show_uuid: bool) {
    ...
    let uuid_str = format!(" {}", task.task_uuid.0.to_string().dimmed());

    println!("  {} {} {}", status_color, symbol, uuid_str);
